...
bool KannalaBrandt8::epipolarConstrain_(
  GeometricCamera* pCamera2,
  const cv::KeyPoint& kp1,
  const cv::KeyPoint& kp2,
...
float KannalaBrandt8::TriangulateMatches_(
  GeometricCamera* pCamera2,
  const cv::KeyPoint& kp1,
  const cv::KeyPoint& kp2,
  const cv::Matx33f& R12,
  ...
  cv::Matx44f Tcw2{R21(0, 0), R21(0, 1), R21(0, 2), t21(0), R21(1, 0), R21(1, 1), R21(1, 2), t21(1), R21(2, 0), R21(2, 1), R21(2, 2), t21(2), 0.f, 0.f, 0.f, 1.f};
...
void KannalaBrandt8::Triangulate(const cv::Point2f& p1, const cv::Point2f& p2, const cv::Mat& Tcw1, const cv::Mat& Tcw2, cv::Mat& x3D) {
  ...
  x3D = vt.row(3).t();
  x3D = x3D.rowRange(0, 3) * (1.0 / x3D.at<float>(3));
}
...
void KannalaBrandt8::Triangulate_(const cv::Point2f& p1, const cv::Point2f& p2, const cv::Matx44f& Tcw1, const cv::Matx44f& Tcw2, cv::Matx31f& x3D) {
  ...
  cv::Matx44f A{A0(0), A0(1), A0(2), A0(3), A1(0), A1(1), A1(2), A1(3), A2(0), A2(1), A2(2), A2(3), A3(0), A3(1), A3(2), A3(3)};
